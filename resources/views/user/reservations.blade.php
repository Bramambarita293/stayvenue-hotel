...
@section('content')
    <section class="mx-auto max-w-container px-5 py-28 md:px-8">
        <div class="mb-8">
            ...
            <p class="mt-3 text-sm text-stone">Kelola transaksi, pembayaran, dan unduh E-Voucher resmi Anda.</p>
        </div>

        <div class="overflow-hidden rounded-2xl border border-line/70 bg-surface shadow-card">
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead>
                        <tr class="border-b border-line/70 bg-surface-muted text-xs uppercase tracking-wider text-stone">
                        ...
                        @forelse ($reservations as $res)
                            <tr class="transition-colors hover:bg-background/60">
                                <td class="p-5">
                                    ...
                                    <div class="mt-1 text-xs text-stone">{{ $res->created_at->format('d M Y  H:i') }}</div>
                                </td>

                                <td class="p-5">
                                    @if ($res->reservation_type == 'ROOM')
                                        <span class="inline-flex rounded-full bg-gold/15 px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-gold-soft">
                                            Kamar Hotel
                                        </span>
                                    @else
                                        <span class="inline-flex rounded-full bg-surface-deep px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-stone">
                                            Sewa Gedung
                                        </span>
                                    @endif
                                ...
                                <td class="p-5">
                                    ...
                                    @elseif ($res->status == 'PENDING_PAYMENT')
                                        <span class="inline-flex items-center gap-1 rounded-full bg-pending/10 px-3 py-1 text-xs font-semibold text-pending">
                                    ...
                                    @elseif (in_array($res->status, ['CONFIRMED', 'COMPLETED']))
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <a href="{{ route('voucher.download', $res->reservation_code) }}" target="_blank"
                                                class="rounded-full bg-ink px-4 py-2 text-xs font-semibold text-background transition-colors hover:bg-gold-soft hover:text-white">PDF</a>
                                        </div>
                                    @else
                                        <span class="text-xs text-stone/60">-</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="p-14 text-center">
                                    <span class="material-symbols-outlined text-5xl text-stone/40">receipt_long</span>
                                    <p class="mt-4 text-ink">Belum ada riwayat reservasi.</p>
                                    <a href="{{ route('rooms.index') }}" class="mt-2 inline-block text-sm font-semibold text-gold-soft underline underline-offset-4">
