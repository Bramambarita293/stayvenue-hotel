{{-- Banner error booking (check_in/out / event_date). --}}
@if ($errors->has('check_in_date') || $errors->has('check_out_date') || $errors->has('event_date') || $errors->has('number_of_rooms'))
    <div class="flex items-start gap-3 rounded-xl border border-danger/20 bg-danger/5 p-4 text-sm text-danger" role="alert">
        ...
        <div class="flex-1">
            ...
            <ul class="mt-1 list-disc space-y-0.5 pl-5 opacity-90">
                @foreach (['check_in_date', 'check_out_date', 'event_date', 'number_of_rooms'] as $key)
