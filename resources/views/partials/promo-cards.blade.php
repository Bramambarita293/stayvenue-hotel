    ...
    <section class="mx-auto max-w-container px-5 pt-20 md:px-8" aria-label="Promosi spesial">
        ...
        <div class="grid grid-cols-1 gap-8 md:grid-cols-3" data-promo-grid>
            @foreach ($promos as $promo)
                ...
                @endphp
                <article
                    class="promo-tilt group relative flex flex-col overflow-hidden rounded-2xl border border-line/70 shadow-card transition-shadow hover:shadow-cardhover {{ $dark ? 'bg-onyx text-background' : 'bg-surface text-ink' }} {{ $expired ? 'opacity-60' : '' }}"
                    data-promo-card @if (!empty($promo['valid_until'])) data-valid-until="{{ $promo['valid_until'] }}" @endif>
                    <div
                        class="pointer-events-none absolute -right-16 -top-16 h-48 w-48 rounded-full bg-gold/20 blur-3xl transition-opacity group-hover:opacity-100">
                    </div>
                    <div class="flex flex-1 flex-col p-7 md:p-8">
                        <div class="flex items-center justify-between gap-3">
                            <span
                                class="inline-flex items-center gap-1.5 rounded-full bg-gold/15 px-3 py-1 text-[11px] font-bold uppercase tracking-widest text-gold-soft">
                            ...
                            @elseif ($expired)
                                <span
                                    class="rounded-full bg-surface-muted px-3 py-1 text-[11px] font-bold uppercase tracking-widest text-stone">Berakhir</span>
                            @endif
                        </div>

                        <h3 class="mt-5 font-display text-2xl font-medium leading-snug">{{ $promo['title'] }}</h3>
                        <p class="mt-3 flex-1 text-sm leading-relaxed {{ $dark ? 'text-background/70' : 'text-stone' }}">
                            {{ $promo['desc'] }}</p>

                        @if (!empty($promo['valid_until']) && !$expired)
                            <p class="mt-3 text-xs {{ $dark ? 'text-background/50' : 'text-stone/70' }}">
                        ...
                        <a href="{{ route($promo['cta_route']) }}" @if ($expired) aria-disabled="true" tabindex="-1" @endif
                            class="mt-6 inline-flex items-center justify-center gap-2 rounded-full px-6 py-3 text-sm font-semibold transition-all hover:-translate-y-0.5 {{ $dark ? 'bg-gold text-white hover:bg-background hover:text-onyx' : 'bg-ink text-background hover:bg-gold-soft hover:text-white' }} {{ $expired ? 'pointer-events-none opacity-50' : '' }}">
                            {{ $promo['cta_label'] }}
