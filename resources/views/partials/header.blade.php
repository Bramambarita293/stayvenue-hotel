<header id="top-nav" class="fixed top-0 inset-x-0 z-50 border-b border-line/70 bg-background/90 backdrop-blur-md transition-all duration-300">
    <div class="mx-auto flex h-20 max-w-container items-center justify-between px-5 md:px-8">
        ...
        <div class="hidden items-center gap-5 md:flex">
            @auth
                <span class="text-sm text-stone">Halo, <span class="font-semibold text-ink">{{ Auth::user()->name }}</span></span>
                <form action="{{ route('logout') }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="text-sm font-medium text-stone transition-colors hover:text-danger">Logout</button>
                </form>
            @else
                <a href="{{ route('login') }}"
                    class="text-sm font-medium text-ink underline-offset-4 transition-colors hover:text-gold-soft hover:underline">Sign in</a>
                <a href="{{ route('register') }}"
                    class="rounded-full bg-ink px-5 py-2.5 text-sm font-semibold text-background transition-all hover:-translate-y-0.5 hover:bg-gold-soft hover:text-white">Book now</a>
            @endauth
        </div>
    ...
    <div id="mobile-menu" class="hidden border-t border-line/70 bg-background px-5 py-4 md:hidden">
