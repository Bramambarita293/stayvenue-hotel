...
@section('content')
    ...
    <section class="relative overflow-hidden bg-onyx pb-16 pt-36 text-background">
        <div class="mx-auto max-w-container px-5 md:px-8">
            <p class="eyebrow text-gold">We are here for you</p>
    ...
    @if (session('success'))
        <div class="mx-auto mt-8 max-w-container px-5 md:px-8">
            <div class="flex items-start gap-3 rounded-xl border border-successful/30 bg-successful/10 p-4 text-sm text-successful">
    ...
    <section class="mx-auto max-w-container px-5 py-16 md:px-8 md:py-24">
        <div class="grid grid-cols-1 gap-12 lg:grid-cols-5">
                ...
                <div class="mt-8 space-y-6">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm leading-relaxed text-stone">{{ $site['address'] }}</p>
                            <a href="{{ $site['map_link'] }}" target="_blank" rel="noopener"
                                class="mt-1 inline-flex items-center gap-1 text-sm font-medium text-gold-soft hover:underline">
                                Buka di Google Maps
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-stone">
                                <a href="tel:{{ preg_replace('/[^+\d]/', '', $site['phone']) }}"
                                    class="hover:text-gold-soft">{{ $site['phone'] }}</a>
                            </p>
                    ...
                    <div class="flex items-start gap-3">
                        <span class="material-symbols-outlined mt-0.5 text-[20px] text-gold">mail</span>
                        <div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-stone">{{ $site['hours'] }} · Restoran 06:00 – 23:00 WIB</p>
                        </div>
                    </div>
                </div>

                <!-- Sosial media -->
                <div class="mt-10 border-t border-line/70 pt-8">
                    <p class="text-xs font-semibold uppercase tracking-[0.2em] text-gold">Ikuti kami</p>
                    <div class="mt-4 flex flex-wrap gap-3">
                        @if (!empty($site['socials']['instagram']))
                            <a href="{{ $site['socials']['instagram'] }}" target="_blank" rel="noopener"
                                class="inline-flex items-center gap-2 rounded-full border border-line bg-surface px-4 py-2 text-sm font-medium text-ink transition-colors hover:border-ink">
                                Instagram
                            </a>
                        @endif
            ...
            <!-- Form pesan -->
            <div class="rounded-2xl border border-line/70 bg-surface p-7 shadow-card lg:col-span-3 md:p-10">
                <h3 class="font-display text-2xl font-medium tracking-tight text-ink">Kirim pesan</h3>
                <p class="mt-2 text-sm text-stone">Isi formulir di bawah — kami biasanya membalas dalam beberapa jam kerja.</p>

                <form action="{{ route('contact.store') }}" method="POST" class="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2">
                    @csrf
                    <div>
                        <label for="name" class="block text-[11px] font-semibold uppercase tracking-wider text-stone">Nama lengkap *</label>
                        <input type="text" id="name" name="name" value="{{ old('name') }}" required maxlength="100"
                            placeholder="Nama Anda"
                            class="mt-1.5 w-full rounded-lg border border-line bg-background px-4 py-2.5 text-sm outline-none transition-colors focus:border-gold focus:ring-1 focus:ring-gold/30" />
                    </div>
                    <div>
                        <label for="email" class="block text-[11px] font-semibold uppercase tracking-wider text-stone">Email *</label>
                        <input type="email" id="email" name="email" value="{{ old('email') }}" required maxlength="150"
                            placeholder="nama@email.com"
                            class="mt-1.5 w-full rounded-lg border border-line bg-background px-4 py-2.5 text-sm outline-none transition-colors focus:border-gold focus:ring-1 focus:ring-gold/30" />
                    </div>
                    <div>
                        <label for="phone" class="block text-[11px] font-semibold uppercase tracking-wider text-stone">No. Telepon / WA</label>
                        <input type="tel" id="phone" name="phone" value="{{ old('phone') }}" maxlength="25"
                            placeholder="+62 8xx xxxx xxxx"
                            class="mt-1.5 w-full rounded-lg border border-line bg-background px-4 py-2.5 text-sm outline-none transition-colors focus:border-gold focus:ring-1 focus:ring-gold/30" />
                    </div>
                    <div>
                        <label for="subject" class="block text-[11px] font-semibold uppercase tracking-wider text-stone">Subjek</label>
                    ...
                    <div class="sm:col-span-2">
                        <label for="message" class="block text-[11px] font-semibold uppercase tracking-wider text-stone">Pesan Anda *</label>
                        <textarea id="message" name="message" rows="6" required minlength="10" maxlength="2000"
                            ...
                            class="mt-1.5 w-full resize-y rounded-lg border border-line bg-background px-4 py-3 text-sm outline-none transition-colors focus:border-gold focus:ring-1 focus:ring-gold/30">{{ old('message') }}</textarea>
                    </div>
                    <div class="sm:col-span-2">
                        <button type="submit"
                            class="inline-flex w-full items-center justify-center gap-2 rounded-full bg-ink px-8 py-3.5 text-sm font-semibold text-background transition-all hover:-translate-y-0.5 hover:bg-gold-soft hover:text-white sm:w-auto">
                            Kirim Pesan
    ...
    <!-- Peta lokasi -->
    <section class="border-y border-line/70 bg-surface-muted">
        <div class="mx-auto max-w-container px-5 py-16 md:px-8 md:py-20">
            <div class="mb-10 flex flex-col justify-between gap-4 md:flex-row md:items-end">
                <div>
                    ...
                    <p class="mt-3 max-w-xl text-sm leading-relaxed text-stone">
            ...
            <div class="overflow-hidden rounded-2xl border border-line/70 shadow-card">
        ...
        <div class="mx-auto max-w-3xl">
            ...
            <div class="mt-10 space-y-4">
                @forelse ($site['faqs'] as $faq)
                    <details class="group rounded-xl border border-line/70 bg-surface transition-shadow open:shadow-card">
                        <summary class="flex cursor-pointer list-none items-center justify-between gap-4 px-6 py-5 [&::-webkit-details-marker]:hidden">
                            ...
                            <span class="material-symbols-outlined shrink-0 text-stone transition-transform duration-300 group-open:rotate-180 group-open:text-gold">expand_more</span>
                        </summary>
                        <p class="px-6 pb-6 text-sm leading-relaxed text-stone">{{ $faq['a'] }}</p>
                    </details>
                @empty
