...
@section('content')
    <section class="bg-onyx pb-16 pt-36 text-background">
        <div class="mx-auto max-w-container px-5 md:px-8">
            <p class="eyebrow text-gold">Unforgettable gatherings</p>
    ...
    <section class="mx-auto max-w-container px-5 py-16 md:px-8 md:py-20">
        @if ($halls->isEmpty())
            <div class="rounded-2xl border border-line/70 bg-surface py-20 text-center">
                <span class="material-symbols-outlined text-5xl text-stone/40">meeting_room</span>
                    ...
                    <div class="flex flex-col gap-8 overflow-hidden rounded-2xl border border-line/70 bg-surface shadow-card md:flex-row {{ $reverse ? 'md:flex-row-reverse' : '' }}">
                        ...
                        <div class="flex flex-1 flex-col justify-center p-7 md:p-10">
                            ...
                            <a href="{{ route('halls.show', $hall->id) }}"
                                class="mt-8 inline-flex w-fit items-center gap-2 rounded-full bg-ink px-6 py-3 text-sm font-semibold text-background transition-all hover:-translate-y-0.5 hover:bg-gold-soft hover:text-white">
